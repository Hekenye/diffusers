...
# TODO: refactor log_validaion for sdxl
...
# TODO: refactor save_progress for sdxl
...
def main():
    ...
    # Load tokenizer
    assert args.tokenizer_name == None, "for sdxl, only use from pretrained"
    ...
    # add dict item for two tokenizer and check the validation of placeholder
    ...
    if num_added_tokens_one != args.num_vectors or num_added_tokens_two != args.num_vectors:
        raise ValueError(
    ...
    if len(token_ids_one) > 1 or len(token_ids_two) > 1:
    ...
    #     [
    #         {'params':text_encoder_one.get_input_embeddings().parameters()},
    #         {'params':text_encoder_two.get_input_embeddings().parameters()}
    ...
    # # same method to load dataset with sdxl_lora
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder_one):
                ...
                # sdxl use the trick named noise_offset
                if args.noise_offset:
                    # https://www.crosslabs.org//blog/diffusion-with-offset-noise
                    noise += args.noise_offset * torch.randn(
                        (latents.shape[0], latents[1], 1, 1), device=model_input.device
                    )
                bsz = latents.shape[0]
                ...
                # insert sdxl conditions and predict the noise residual
                ...
                # FIXME: for test: make sure we update the embedding of placeholder_tokens
                index_updates_one = torch.ones((len(tokenizer_one),), dtype=torch.bool)
                index_updates_one[:min(placeholder_token_ids_one)] = False
                
                assert accelerator.unwrap_model(text_encoder_one).get_input_embeddings().weight[
                    index_updates_one
                    ].equal(orig_embeds_params_one[index_updates_one])==False ,\
                print("the embedding of placeholder token is not update")
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    save_path = os.path.join(args.output_dir, f"learned_embeds_one-steps-{global_step}.bin")
                    save_progress(text_encoder_one, placeholder_token_ids_one, accelerator, args, save_path)
                    
                    save_path = os.path.join(args.output_dir, f"learned_embeds_two-steps-{global_step}.bin")
                    save_progress(text_encoder_two, placeholder_token_ids_two, accelerator, args, save_path)
                ...
                if accelerator.is_main_process:
                    ...
                    # FIXME: now the log_validation could not work
    ...
    if accelerator.is_main_process:
        ...
        save_path = os.path.join(args.output_dir, "learned_embeds_one.bin")
        save_progress(text_encoder_one, placeholder_token_ids_one, accelerator, args, save_path)
        save_path = os.path.join(args.output_dir, "learned_embeds_two.bin")
        save_progress(text_encoder_two, placeholder_token_ids_two, accelerator, args, save_path)
        if args.push_to_hub:
            save_model_card(
                repo_id,
